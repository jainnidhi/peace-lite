 ...
 *
 * @package WordPress
 * @subpackage Peace Theme
 */

// This is the URL our updater / license checker pings. This should be the URL of the site with Peace installed
define('IDEABOX_STORE_URL', 'http://ideaboxthemes.com'); // add your own unique prefix to prevent conflicts
// The name of your product. This should match the download name in Peace exactly
define('IDEABOX_THEME_NAME', 'Peace One Page WordPress Theme'); // add your own unique prefix to prevent conflicts

function ideabox_theme_updater() {

    $test_license = trim(get_option('ideabox_theme_license_key'));

    $peace_updater = new peace_SL_Theme_Updater(array(
        'remote_api_url' => IDEABOX_STORE_URL, // Our store URL that is running Peace
        'version' => '1.0.1', // The current theme version we are running
...
function ideabox_theme_license_page() {
    ...
    ?>
    <div class="wrap">
        <h2><?php _e('Theme License Options','peace'); ?></h2>
    ...
    <?php _e('License Key','peace'); ?>
                        ...
                        <td>
                            ...
                            <label class="description" for="ideabox_theme_license_key"><?php _e('Enter your license key','peace'); ?></label>
                        </td>
                    </tr>
    <?php if (false !== $license) { ?>
                        <tr valign="top">
                            <th scope="row" valign="top">
        <?php _e('Activate License','peace'); ?>
                            </th>
                            <td>
        <?php if ($status !== false && $status == 'valid') { ?>
                                    <span style="color:green;"><?php _e('active','peace'); ?></span>
                                    <?php wp_nonce_field('peace_sample_nonce', 'peace_sample_nonce'); ?>
                                    <input type="submit" class="button-secondary" name="peace_theme_license_deactivate" value="<?php _e('Deactivate License','peace'); ?>"/>
                                <?php } else {
                                    wp_nonce_field('peace_sample_nonce', 'peace_sample_nonce');
                                    ?>
                                    <input type="submit" class="button-secondary" name="peace_theme_license_activate" value="<?php _e('Activate License','peace'); ?>"/>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
    ...
    function ideabox_theme_register_option() {
        ...
        register_setting('ideabox_theme_license', 'ideabox_theme_license_key', 'peace_theme_sanitize_license');
    }
    ...
    function ideabox_theme_license_menu() {
        add_theme_page('Theme License', 'Theme License', 'manage_options', 'peace-license', 'ideabox_theme_license_page');
    }
    ...
    function peace_theme_sanitize_license($new) {
    ...
    function ideabox_theme_activate_license() {
        ...
        if (isset($_POST['peace_theme_license_activate'])) {
            if (!check_admin_referer('peace_sample_nonce', 'peace_sample_nonce'))
    ...
    function ideabox_theme_deactivate_license() {
        ...
        if (isset($_POST['peace_theme_license_deactivate'])) {
...
// retrieve the license from the database
            ...
            $api_params = array(
                ...
                'license' => $license,
                'item_name' => urlencode(IDEABOX_THEME_NAME) // the name of our product in Peace
            );
