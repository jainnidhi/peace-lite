...
/* 
 * Peace extra functions that extend the look and feel 
 * of the theme. 
 * 
 * @package Peace
 * @since Peace 1.0 
 ...
 */
function peace_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'peace' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'peace_wp_title', 10, 2 );
 ...
 * @return void
 */
if ( ! function_exists( 'peace_content_nav' ) ) {
	function peace_content_nav( $nav_id ) {
		global $wp_query;
		...
		<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
			<h3 class="assistive-text"><?php esc_html_e( 'Post navigation', 'peace' ); ?></h3>

			<?php if ( is_single() ) { // navigation links for single posts ?>

				<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-angle-left"></i>', 'Previous post link', 'peace' ) . '</span> %title' ); ?>
				<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '<i class="fa fa-angle-right"></i>', 'Next post link', 'peace' ) . '</span>' ); ?>
			...
			elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) { // navigation links for home, archive, and search pages ?>
				...
				<?php echo paginate_links( array(
					...
					'type' => 'list',
					'prev_text' => wp_kses( __( '<i class="fa fa-angle-left"></i> Previous', 'peace' ), array( 'i' => array( 
						'class' => array() ) ) ),
					'next_text' => wp_kses( __( 'Next <i class="fa fa-angle-right"></i>', 'peace' ), array( 'i' => array( 
						'class' => array() ) ) )
				) ); ?>
 ...
 * simply create your own peace_comment(), and that function will be used instead.
 *
 ...
 * @return void
 */
if ( ! function_exists( 'peace_comment' ) ) {
	function peace_comment( $comment, $args, $depth ) {
		...
		case 'trackback' :
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				<article id="comment-<?php comment_ID(); ?>" class="pingback">
					<p><?php esc_html_e( 'Pingback:', 'peace' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'peace' ), '<span class="edit-link">', '</span>' ); ?></p>
				...
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<header class="comment-meta comment-author vcard">
						...
						printf( '<cite class="fn">%1$s %2$s</cite>',
							...
							( $comment->user_id === $post->post_author ) ? '<span> ' . esc_html__( 'Post author', 'peace' ) . '</span>' : '' );
						printf( '<a href="%1$s" title="Posted %2$s"><time itemprop="datePublished" datetime="%3$s">%4$s</time></a>',
							esc_url( get_comment_link( $comment->comment_ID ) ),
							sprintf( esc_html__( '%1$s @ %2$s', 'peace' ), esc_html( get_comment_date() ), esc_attr( get_comment_time() ) ),
							get_comment_time( 'c' ),
							/* Translators: 1: date, 2: time */
							sprintf( esc_html__( '%1$s at %2$s', 'peace' ), get_comment_date(), get_comment_time() )
						);
						?>
					</header> <!-- .comment-meta -->

					<?php if ( '0' == $comment->comment_approved ) { ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'peace' ); ?></p>
					<?php } ?>
					...
					<div class="reply">
						<?php comment_reply_link( array_merge( $args, array( 'reply_text' => wp_kses( __( 'Reply <span>&darr;</span>', 'peace' ), array( 'span' => array() ) ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
					</div> <!-- .reply -->
				</article> <!-- #comment-## -->
			<?php
			break;
...
if ( ! function_exists( 'peace_posted_on' ) ) {
	function peace_posted_on() {
		...
		$date = sprintf( '<i class="fa %1$s"></i> <a href="%2$s" title="Posted %3$s" rel="bookmark"><time class="entry-date" datetime="%4$s" itemprop="datePublished">%5$s</time></a>',
			...
			sprintf( esc_html__( '%1$s @ %2$s', 'peace' ), esc_html( get_the_date() ), esc_attr( get_the_time() ) ),
		...
		$author = sprintf( '<i class="fa fa-user"></i> <address class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></address>',
			...
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'peace' ), get_the_author() ) ),
			get_the_author()
		);
		...
		$comments = sprintf( '<span class="comments-link"><i class="fa fa-comment"></i> <a href="%1$s" title="%2$s">%3$s</a></span>',
			...
			( get_comments_number() > 0 ? sprintf( _n( '%1$s Comment', '%1$s Comments', get_comments_number() ), get_comments_number() ) : esc_html__( 'No Comments', 'peace' ) )
		);

		// Translators: 1: Date 2: Author 3: Categories 4: Comments
		printf( wp_kses( __( '<div class="header-meta">%1$s%2$s<span class="post-categories">%3$s</span>%4$s</div>', 'peace' ), array( 
			'div' => array ( 
				'class' => array() ), 
			'span' => array( 
				'class' => array() ) ) ),
...
function peace_post_format_icon() { 
 ...
 * @return void
 */
if ( ! function_exists( 'peace_entry_meta' ) ) {
	function peace_entry_meta() {
		// Return the Tags as a list
		$tag_list = "";
		if ( get_the_tag_list() ) {
			$tag_list = get_the_tag_list( '<span class="post-tags">', esc_html__( ' ', 'peace' ), '</span>' );
		}

		// Translators: 1 is tag
		if ( $tag_list ) {
			printf( wp_kses( __( '<i class="fa fa-tag"></i> %1$s', 'peace' ), array( 'i' => array( 'class' => array() ) ) ), $tag_list );
		}
	}
}
 ...
 */
function peace_new_contactmethods( $contactmethods ) {
	// Add Twitter
...
add_filter( 'user_contactmethods', 'peace_new_contactmethods', 10, 1 );
 ...
 */
function peace_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'peace_add_menu_parent_class' );
...
function peace_body_classes($classes) {
    ...
    if(get_theme_mod('peace_color_scheme')) {
        $slug = strtolower(get_theme_mod('peace_color_scheme'));
        $classes[] = 'peace-' . $slug;
        
    }
    
    $bg_slug = get_theme_mod('peace_bg_color_scheme');
    if (is_front_page()) {
        $classes[] = $bg_slug . '-peace-front-page';
    }
    $classes[] = 'peace-bg-'.$bg_slug; 
...
add_action( 'after_setup_theme', 'peace_envira_define_license_key' );
function peace_envira_define_license_key() {
 ...
 */
global $pagenow;
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) add_action( 'init', 'peace_woocommerce_image_dimensions', 1 );
 
/**
 * Define image sizes
 */
function peace_woocommerce_image_dimensions() {
  	$catalog = array(
		'width' 	=> '349',	// px
		'height'	=> '349',	// px
		'crop'		=> 1 		// true
 ...
 */
function peace_featured_video( $content ) {
...
function peace_video_content( $content ) {
	$url = trim( current ( explode( "\n", $content ) ) );	
	if ( 0 === strpos( $url, 'http://' ) || preg_match ( '#^<(script|iframe|embed|object)#i', $url )) { 		
...
function peace_flexslider($size = thumbnail) {
	...
	$images = get_posts(array(
		...
		<div id ="gallery-content" class="flexslider">
...
if( !function_exists ('peace_extract_from_string') ) :
function peace_extract_from_string($start, $end, $tring) {
